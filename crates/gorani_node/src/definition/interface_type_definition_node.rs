...
pub struct InterfaceTypeDefinitionNode(pub(crate) SyntaxNode);

impl Node for InterfaceTypeDefinitionNode {
    fn cast(node: SyntaxNode) -> Option<Self> {
        if matches!(node.kind(), SyntaxKind::INTERFACE_TYPE_DEFINITION) {
            Some(Self(node))
        } else {
            None
        }
...
impl Parent<InterfaceTypeDefinitionNode> for crate::TypeDefinitionNode {
    //
}
